...
class EPKB_Articles_CPT_Setup
{
	...
	public function __construct()
	{
		...
		add_action('init', array($this, 'insert_default_article_cat_term'), 10);
		//add_action('init', array($this, 'set_default_article_cat'), 10);
	...
	public function insert_default_article_cat_term()
	{
		// check if category(term) exists
		$cat_exists = term_exists('default_epkb_post_type_1_category', 'epkb_post_type_1_category');
		...
		if (!$cat_exists) {
			...
			$new_cat = wp_insert_term(
				...
				array(
					'description'   =>  'This is your default category',
					'slug'          =>  'default_epkb_post_type_1_category',
				)
			);
		...
		$stored_default_cat = get_option('default_epkb_post_type_1_category');

		if (empty($stored_default_cat) && $default_cat_id)
			update_option('default_epkb_post_type_1_category', $default_cat_id);
	}
	// public function set_default_article_cat($post_id, $post)
	// {
	// 	if ('publish' === $post->post_status) {
	// 		$gallery_cats = wp_get_post_terms($post_id, 'epkb_post_type_1_category');
	// 		$default_gallery_cat = (int) get_option('default_epkb_post_type_1_category');

	// 		if (empty($gallery_cats)) {
	// 			wp_set_object_terms($post_id, $default_gallery_cat, 'epkb_post_type_1_category');
	// 		}
	// 	}
	// }
